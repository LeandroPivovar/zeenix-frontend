<template>
  <div v-if="visible" class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      <h3 class="modal-title">Alterar Telefone</h3>
      <div class="form-group">
        ...
        />
        <small class="help-text">Insira o número com código do país e DDD.</small>
      </div>
      <div class="modal-actions">
        <button class="btn-cancel" @click="$emit('close')">Cancelar</button>
        <button class="btn-save" @click="handleSave" :disabled="!isValidPhone">
          Salvar
        </button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    isValidPhone() {
      // Basic validation: at least 8 chars, allows numbers, spaces, +, -
      const phoneRegex = /^[\d\s+-]{8,}$/
      return this.phone && phoneRegex.test(this.phone)
    }
  },
  watch: {
    visible(newVal) {
...
.help-text { display: block; margin-top: 4px; font-size: 11px; color: #6b7280; }
