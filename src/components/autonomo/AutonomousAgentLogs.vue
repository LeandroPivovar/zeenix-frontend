        ...
        <div class="h-full w-full overflow-y-auto custom-scrollbar" ref="desktopLogsContainer">
          <div class="p-4 font-mono text-xs space-y-4">
            ...
            <div 
              ...
              :key="log.id || index"
              class="space-y-0.5 animate-in fade-in slide-in-from-left-1 duration-200"
            >
              <div class="flex items-start gap-2">
                <span class="text-[#A1A1AA] shrink-0 font-mono">[{{ formatTimestamp(log.timestamp) }}]</span>
                <div :class="['space-y-0.5', getLogColorClass(log.logType)]">
                  ...
                  <div v-if="log.details" class="pl-5 text-[#A1A1AA] text-[10px] leading-relaxed">
                    {{ log.details }}
                  </div>
                  <!-- Special handling for multi-line analysis or config if available -->
                   <div v-if="log.message && log.message.includes('\n')" class="pl-5 space-y-0.5">
                      <div v-for="(line, lIdx) in log.message.split('\n').slice(1)" :key="lIdx" class="text-[10px] leading-relaxed">
                        • {{ line.trim() }}
                      </div>
                   </div>
                </div>
