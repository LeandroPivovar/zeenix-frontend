    ...
    <div class="dashboard-layout">
        ...
        <div class="dashboard-content-wrapper" :class="{ 'sidebar-collapsed': isSidebarCollapsed }">
            ...
            <main class="layout-content">
                <div class="content-header mb-6 flex justify-between items-center px-4">
                    <div>
                        <h1 class="text-2xl font-bold text-white">{{ isMonitoring ? 'Acompanhamento de Estrat├®gia [BETA]' : 'Criador de Estrat├®gias [BETA]' }}</h1>
                        <p class="text-sm text-[#7D7D7D]">{{ isMonitoring ? 'Acompanhe a atividade do rob├┤ em tempo real.' : 'Configure sua estrat├®gia automatizada para execu├º├úo no mercado.' }}</p>
                    </div>
                    <button v-if="isMonitoring" @click="stopMonitoring" class="stop-btn">
                        <i class="fas fa-stop mr-2"></i> Parar Rob├┤
                    </button>
                    <div v-else class="flex flex-wrap items-center gap-3">
                        ...
                        <div class="balance-card">
                            <span class="text-[10px] uppercase text-[#7D7D7D] font-bold">Saldo Dispon├¡vel</span>
                ...
                <div v-if="isMonitoring" class="monitoring-dashboard animate-fadeIn px-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div class="stats-card">
                            ...
                            <div class="stats-info">
                                <span class="stats-label">Sess├úo P/L</span>
                        ...
                        <div v-if="sessionState.isRecoveryMode" class="stats-card border border-zenix-green/30 bg-zenix-green/5">
                            ...
                            <div class="stats-info">
                                <span class="stats-label">Recupera├º├úo</span>
                    ...
                    <div class="monitoring-tabs-container mb-4">
                        <div class="monitoring-tabs flex gap-4 border-b border-[#333]">
                            <button @click="activeMonitoringTab = 'chart'" :class="{ 'active text-zenix-green border-b-2 border-zenix-green': activeMonitoringTab === 'chart' }" class="pb-2 px-4 transition-all hover:text-white text-[#7A7A7A]">
                                <i class="fas fa-chart-area mr-2"></i> Gr├ífico
                            </button>
                    ...
                    <div class="tab-content-container bg-[#141414] border border-[#333] rounded-xl p-6 min-h-[400px]">
                        ...
                        <div v-show="activeMonitoringTab === 'chart'" class="chart-tab-content flex items-center justify-center h-full min-h-[300px]">
                            <div class="text-center">
                                ...
                                <p class="text-[#7A7A7A]">Aguardando conex├úo com o mercado...</p>
                            </div>
                        </div>
                                ...
                                <table class="w-full text-left">
                                    ...
                                    <tbody class="text-sm">
                                        ...
                                        <tr v-if="monitoringOperations.length === 0">
                                            <td colspan="6" class="text-center py-12 text-[#7A7A7A]">Nenhuma opera├º├úo executada nesta sess├úo.</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                ...
                <div v-else class="strategy-creator-form-container px-4">
                    <form @submit.prevent="submitForm" class="space-y-8">
                        ...
                        <!-- Tipo de Negocia├º├úo -->
                        <div class="form-group col-span-12">
                            <label class="block text-white font-bold mb-2">Tipo de Negocia├º├úo</label>
                            <button
                                type="button"
                        ...
                        <!-- Dura├º├úo, Unidade, Multiplicador -->
                        <div class="col-span-12">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    <label class="block text-white font-bold mb-2">Dura├º├úo</label>
                                    <input 
                                        type="number" 
                        ...
                        <div class="col-span-12" v-if="['DIGITOVER', 'DIGITUNDER', 'DIGITMATCH', 'DIGITDIFF'].includes(form.tradeType)">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    <label class="block text-white font-bold mb-2">D├¡gito Alvo (Previs├úo)</label>
                                    <div class="relative">
                                        <select 
                            ...
                            <div class="bg-[#141414] border border-[#333] rounded-xl p-6 relative overflow-hidden">
                                ...
                                <div class="space-y-4 relative z-10">
                                    <div v-if="form.attackFilters.length === 0" class="p-4 bg-[#1E1E1E] border border-dashed border-[#444] rounded-lg text-center">
                                        <p class="text-gray-400 text-sm mb-3">Nenhum filtro de ataque configurado. O rob├┤ entrar├í em cada sinal dispon├¡vel.</p>
                                        <button 
                                            type="button" 
                                    ...
                                    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div v-for="filter in form.attackFilters" :key="filter.id" class="p-4 bg-[#1E1E1E] border border-[#333] rounded-lg flex items-center justify-between">
                                            <div>
                                                ...
                                                <span class="text-[10px] text-gray-400">Ativo ÔÇó Configurado</span>
                                            </div>
                        ...
                        <!-- Configura├º├úo de Recupera├º├úo (New Section) -->
                        <div class="col-span-12">
                            <div class="bg-[#141414] border border-[#333] rounded-xl p-6 relative overflow-hidden">
                                <div class="absolute top-0 right-0 p-4 opacity-5">
                                ...
                                <h3 class="text-xl font-bold text-white mb-4 relative z-10 flex items-center justify-between gap-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        Configura├º├úo de Recupera├º├úo
                                    </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10" :class="{ 'opacity-40 grayscale pointer-events-none': !recoveryConfig.enabled }">
                                    <div>
                                        <label class="block text-white font-bold mb-2">Mercado de Recupera├º├úo</label>
                                        <button
                                            type="button"
                                    ...
                                    <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div v-if="['DIGITOVER', 'DIGITUNDER', 'DIGITMATCH', 'DIGITDIFF'].includes(recoveryConfig.tradeType)">
                                            <label class="block text-white font-bold mb-2">D├¡gito Alvo Rec.</label>
                                            <div class="relative">
                                                <select 
                                    ...
                                    <div class="md:col-span-2 flex gap-4">
                                        ...
                                        >
                                         ...
                                         <button 
                                            ...
                                            class="flex-1 bg-[#2A2A2A] hover:bg-[#333] text-white py-3 rounded-lg border border-[#444] font-medium transition-colors flex items-center justify-center gap-2"
                                        >
                                            <i class="fa-solid fa-pause"></i> Pausa Estrat├®gia
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-span-12">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <div class="relative">
                                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-white font-bold">├É</span>
                                        <input 
                                            type="number" 
                                ...
                                <div>
                                    ...
                                    <div v-if="form.useBlindado" class="mt-2 flex items-center gap-2">
                                        <label class="text-xs text-gray-400">Piso Prote├º├úo:</label>
                            ...
                            <div v-for="(marketsList, category) in marketsByCategory" :key="category" class="category-card">
                                <div class="category-card-header">
                                    <div class="category-icon-wrapper">
                                        <svg v-if="category === '├ìndices Cont├¡nuos'" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <Teleport to="body">
            ...
            >
                <div class="modal-content categorized-modal">
                    <div class="modal-header">
                        <h3 class="modal-title">Selecionar Tipo de Negocia├º├úo</h3>
                ...
                <div class="modal-content categorized-modal" style="max-width: 600px">
                    <div class="modal-header">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="modal-title font-bold text-white">{{ filterStep === 1 ? (modalContext === 'main' ? 'Selecionar Filtros de Ataque' : 'Selecionar Filtros de Recupera├º├úo') : 'Configurar Filtros' }}</h3>
                        </div>
                    ...
                    <div class="modal-body custom-scrollbar" style="max-height: 70vh; overflow-y: auto;">
                        <!-- Step 1: Selection -->
                         <div v-if="filterStep === 1" class="space-y-4">
                            <p class="text-sm text-gray-400 mb-4 px-1">Selecione at├® 2 filtros para as entradas de {{ modalContext === 'main' ? 'ataque' : 'recupera├º├úo' }}.</p>
                            ...
                            <div class="mt-8">
                                ...
                                >
                                    <span>Continuar para Configura├º├úo</span>
                        ...
                        <div v-else class="space-y-6 animate-fadeIn">
                            <div v-for="filter in activeFiltersForModal.filter(f => f.active)" :key="filter.id" class="p-6 bg-[#0B0B0B] border border-[#333] rounded-xl space-y-4">
                                ...
                                <div v-if="filter.id === 'digit_density'" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-[10px] text-gray-500 uppercase mb-1 font-bold">Janela (Ticks/D├¡gitos)</label>
                                    ...
                                    <div>
                                        <label class="block text-[10px] text-gray-500 uppercase mb-1 font-bold">D├¡gitos Alvo (Sep. por v├¡rgula)</label>
                                ...
                                <div v-if="filter.id === 'digit_sequence'" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-[10px] text-gray-500 uppercase mb-1 font-bold">Tamanho da Sequ├¬ncia</label>
                                    ...
                                    <div>
                                        ...
                                        <select v-model="filter.config.target" class="w-full bg-[#181818] border border-[#333] rounded p-3 text-sm text-white focus:border-zenix-green outline-none">
                                            ...
                                            <option value="odd">├ìmpares</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div v-if="filter.id === 'parity_majority'" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        <label class="block text-[10px] text-gray-500 uppercase mb-1 font-bold">% M├¡nima</label>
                                    ...
                                    <div>
                                        <label class="block text-[10px] text-gray-500 uppercase mb-1 font-bold">M├íx. Ru├¡do</label>
                                ...
                                <div v-if="filter.id === 'price_momentum'" class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    ...
                                    <div>
                                        <label class="block text-[10px] text-gray-500 uppercase mb-1 font-bold">Delta M├¡n.</label>
        ...
        <Teleport to="body">
            <div v-if="showPauseModal" class="modal-overlay" @click.self="showPauseModal = false">
                <div class="modal-content" style="max-width: 500px">
                     <div class="modal-header">
                        <h3 class="modal-title">Pausa Estrat├®gica</h3>
                    ...
                    <div class="modal-body space-y-6">
                        ...
                        <div class="pt-4 border-t border-[#333]">
                            <h4 class="text-lg font-bold text-white mb-3">Filtro de Sa├¡da (Retomar)</h4>
...
export default {
    ...
    data() {
        return {
            ...
            monitoringStats: {
                ...
                statusDesc: 'Analisando condi├º├Áes de mercado'
            },
            monitoringLogs: [],
            ...
            filters: [
                { 
                    id: 'digit_density', 
                    name: 'Densidade de D├¡gitos (Apollo)', 
                    active: false, 
                    type: 'digit',
                    desc: 'Analisa a frequ├¬ncia de d├¡gitos espec├¡ficos numa janela.',
                    config: { window: 20, digits: '8,9', operator: '<', threshold: 5 }
                },
                { 
                    id: 'digit_sequence', 
                    name: 'Sequ├¬ncia de D├¡gitos (Orion)', 
                    active: false, 
                    type: 'digit',
                    desc: 'Busca sequ├¬ncias de d├¡gitos (Par, ├ìmpar, <4, etc).',
                ...
                { 
                    ...
                    active: false, 
                    type: 'digit',
                    desc: 'Exige que um lado (Par/├ìmpar) domine a janela.',
                    config: { window: 24, percentage: 60, maxNoise: 8 }
                },
                { 
                    id: 'price_momentum', 
                    name: 'Momentum de Pre├ºo (Nexus)', 
                    active: false, 
                    type: 'price',
                    desc: 'Analisa a for├ºa e dire├º├úo dos tiques de pre├ºo.',
            ...
            tradeTypeCategories: [
                ...
                },
                {
                    ...
                    items: [
                        ...
                        { value: 'DIGITODD', label: '├ìmpar', icon: 'TradeTypesEvenOddIcon.svg' }
                    ]
                },
                {
                    id: 'over_under',
            ...
            allTradeTypes: [
                ...
                { value: 'DIGITEVEN', label: 'Par' }, { value: 'DIGITODD', label: '├ìmpar' },
                ...
                { value: 'ONETOUCH', label: 'Toca' }, { value: 'NOTOUCH', label: 'N├úo Toca' },
    ...
    computed: {
        marketsByCategory() {
            ...
            const categoryPriority = {
                '├ìndices Cont├¡nuos': 1,
                '├ìndices Daily Reset': 2,
                '├ìndices Step': 3,
                '├ìndices JUMP': 4,
                '├ìndices Crash/Boom': 5,
                'Criptomoedas': 6,
                'Forex Majors': 7,
                'Major Pairs': 7,
                'Forex Minors': 8,
            ...
            const nameMap = {
                'Indices Step': '├ìndices Step',
                'Jump Indices': '├ìndices JUMP',
                'Boom/Crash': '├ìndices Crash/Boom',
                'Daily Reset Indices': '├ìndices Daily Reset',
                'Major Pairs': 'Forex Majors',
                'Minor Pairs': 'Forex Minors',
                'Continuous Indices': '├ìndices Cont├¡nuos'
            };
        ...
        selectedRecoveryMarketLabel() {
            ...
            return marketByValue ? marketByValue.label : 'Mercado de Recupera├º├úo';
        },
    ...
    methods: {
        ...
        toggleFilter(filter) {
            ...
            if (activeCount >= 2) {
                this.$root.$toast.warning('Selecione no m├íximo 2 filtros.');
                return;
            }
        ...
        openTradeTypeModal(context = 'main') {
            ...
            if (!contracts.length) {
                this.$root.$toast.warning('Selecione um mercado v├ílido primeiro.');
                return;
            }
        ...
        getDerivToken() {
            try {
                ...
                // 2. Fallback para deriv_token padr├úo
        ...
        saveCurrentStrategy() {
            const name = prompt('Nome da estrat├®gia:', `Minha Estrat├®gia ${new Date().toLocaleDateString()}`);
            if (!name) return;
            ...
            this.$root.$toast.success('Estrat├®gia salva com sucesso!');
        },
        ...
        loadSavedStrategy() {
            ...
            this.addLog(`­ƒôé Estrat├®gia carregada: ${strategy.name}`, 'info');
            this.$root.$toast.success(`Estrat├®gia "${strategy.name}" carregada!`);
        },
        ...
        deleteSavedStrategy() {
            ...
            this.$root.$toast.info('Estrat├®gia exclu├¡da.');
        },
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (data.config && data.config.form && data.config.recoveryConfig) {
                        ...
                        this.addLog('­ƒôü Estrat├®gia importada com sucesso de arquivo.', 'success');
                        this.$root.$toast.success('Estrat├®gia importada com sucesso!');
                    } else {
                        throw new Error('Formato de arquivo inv├ílido.');
                    }
                } catch (err) {
        ...
        startSimulation() {
            this.addLog('­ƒñû Rob├┤ iniciado. Aguardando conex├úo com mercado...', 'info');
        ...
        async initTickConnection() {
            if (!this.form.market) {
                this.addLog('ÔÜá´©Å Nenhum mercado selecionado.', 'error');
                return;
            }
            ...
            this.ws.onopen = () => {
                this.addLog(`­ƒöî Conectado. Autorizando...`, 'info');
                const token = this.getDerivToken();
                
                if (token) {
                    console.log('[WS] Enviando token de autoriza├º├úo Deriv...');
                    this.ws.send(JSON.stringify({ authorize: token }));
                } else {
                    console.warn('[WS] Token Deriv n├úo encontrado!');
                    this.addLog('ÔÜá´©Å Token Deriv n├úo encontrado. Opera├º├Áes reais desativadas.', 'warning');
                    this.subscribeTicks();
                }
            };
            ...
            this.ws.onmessage = (event) => {
                try {
                    ...
                    if (msg.msg_type === 'authorize') {
                        if (msg.error) {
                            console.error('[WS] Erro na autoriza├º├úo:', msg.error);
                            this.addLog(`ÔØî Falha na autoriza├º├úo: ${msg.error.message}`, 'error');
                        } else {
                            this.isAuthorized = true;
                            this.balance = msg.authorize.balance;
                            console.log('[WS] Autorizado com sucesso! Dados:', msg.authorize);
                            this.addLog(`Ô£à Autorizado! Saldo: $${this.balance}`, 'success');
                            this.subscribeTicks();
                        }
                    }
                    ...
                    if (msg.msg_type === 'buy') {
                        ...
                        if (msg.error) {
                            ...
                            this.addLog(`ÔØî ERRO NA COMPRA: ${msg.error.message}`, 'error');
                        } else {
                            ...
                            this.addLog(`­ƒÜÇ COMPRA REALIZADA! ID: ${msg.buy.contract_id} | Payout: $${payout} (${profitPercent}%)`, 'success');
                            this.subscribeToContract(msg.buy.contract_id);
                        }
                    }

                    if (msg.msg_type === 'proposal_open_contract') {
                        console.log('[WS] Atualiza├º├úo de contrato:', msg.proposal_open_contract);
            ...
            this.ws.onerror = (e) => {
                ...
                this.addLog('ÔØî Erro na conex├úo com o mercado.', 'error');
            };

            this.ws.onclose = () => {
                if (this.isMonitoring) {
                    this.addLog('­ƒôí Conex├úo encerrada. Tentando reconectar...', 'info');
                    setTimeout(() => {
        ...
        handleTickMessage(msg) {
            if (msg.error) {
                this.addLog(`ÔØî Erro Deriv: ${msg.error.message}`, 'error');
                return;
            }
            ...
            if (msg.msg_type === 'tick' && msg.tick) {
                ...
                this.addLog(`­ƒôê Tick recebido: ${price} - Tick #${this.tickCount}`, 'info');
        ...
        runAnalysis() {
            ...
            results.forEach(res => {
                if (!res.pass) {
                    this.addLog(`­ƒöì Analisando: ${res.reason}`, 'info');
                }
            });

            if (allPassed) {
                this.addLog('­ƒÄ» SINAL GERADO! Filtros de ataque confirmados.', 'success');
        ...
        checkLimits() {
            ...
            if (target > 0 && lucroAtual >= target) {
                this.addLog(`­ƒÄ» META ATINGIDA: +$${lucroAtual.toFixed(2)}`, 'success');
            ...
            if (this.form.useBlindado && target > 0) {
                ...
                if (currentPeak >= activationThreshold) {
                    if (!this.sessionState.stopBlindadoActive) {
                        ...
                        this.addLog(`­ƒøí´©Å STOP BLINDADO ATIVADO! Piso: $${this.sessionState.stopBlindadoFloor.toFixed(2)}`, 'info');
                    }

                    if (lucroAtual <= this.sessionState.stopBlindadoFloor) {
                        this.addLog(`­ƒøí´©Å STOP BLINDADO ATINGIDO: Protegendo $${lucroAtual.toFixed(2)}`, 'warning');
            ...
            if (stopLoss > 0 && lucroAtual <= -stopLoss) {
                this.addLog(`­ƒøæ STOP LOSS ATINGIDO: -$${Math.abs(lucroAtual).toFixed(2)}`, 'error');
        ...
        stopMonitoring(reason = 'Finalizado pelo usu├írio') {
            // Handle PointerEvents if called from @click
            const stopReason = (reason instanceof Event) ? 'Finalizado pelo usu├írio' : reason;
            ...
            this.stopTickConnection();
            this.addLog(`ÔÅ╣´©Å Monitoramento parado: ${stopReason}`, 'info');
        },
        executeRealTrade() {
            if (!this.isAuthorized) {
                this.addLog('ÔÜá´©Å Entrada negada: N├úo autorizado (Token inv├ílido ou ausente).', 'warning');
                return;
            }

            if (this.checkLimits()) return;

            // Evitar m├║ltiplas entradas simult├óneas
            if (this.activeContracts.size > 0) {
                this.addLog('ÔÅ│ Sinal ignorado: J├í existe uma opera├º├úo em andamento.', 'info');
                return;
            }
            ...
            this.addLog(`­ƒÆ© Enviando ordem (${this.sessionState.isRecoveryMode ? 'RECUPERA├ç├âO' : 'PRINCIPAL'}): ${config.tradeType} $${stake}`, 'info');
        ...
        handleContractUpdate(contract) {
            ...
            if (contract.is_sold) {
                ...
                if (trade.result === 'WON') {
                    ...
                    this.addLog(`­ƒÆ░ WIN! Resultado: +$${trade.pnl.toFixed(2)} (Stake: $${trade.stake.toFixed(2)})`, 'success');
                    ...
                    if (this.sessionState.isRecoveryMode) {
                        ...
                        if (this.sessionState.recoveredAmount >= this.sessionState.totalLossAccumulated) {
                            this.addLog('Ô£à RECUPERA├ç├âO CONCLU├ìDA!', 'success');
                    ...
                    this.addLog(`­ƒö┤ LOSS! Preju├¡zo: -$${Math.abs(trade.pnl).toFixed(2)} (Stake: $${trade.stake.toFixed(2)})`, 'error');
                    ...
                    if (!this.sessionState.isRecoveryMode && 
                        ...
                        this.addLog('­ƒöä ATIVANDO MODO RECUPERA├ç├âO...', 'warning');
        ...
        simulateLog() {
            const logs = [
                'Analisando tend├¬ncia...',
                'EMA 20 cruzando para cima.',
                'RSI em n├¡vel de neutralidade.',
                'Aguardando confirma├º├úo de RSI...',
                'Filtro de volatilidade OK.',
                'Padr├úo Candlestick detectado.'
            ];
